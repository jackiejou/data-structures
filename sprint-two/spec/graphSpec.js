describe('graph', function() {
  ...
  it('should return the number of edges for a node', function() {
    graph.addNode(4);
    graph.addNode(5);
    graph.addNode(6);
    graph.addNode(7);
    ...
    graph.addEdge(4, 7);
    graph.addEdge(6, 7);
    expect(graph.countEdges(4)).to.equal(2);
    expect(graph.countEdges(6)).to.equal(1);
    expect(graph.countEdges(2)).to.equal(0);
  });
});
